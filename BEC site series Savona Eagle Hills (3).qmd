...
# poly_crs <- st_as_sf(poly) %>% 
#   st_transform(st_crs(bec_ctg))
# poly_crs

# poly_bec <- st_intersection(bec_ctg, poly) %>% 
#   group_by(MAP_LABEL)

vectors <- vect(poly)

extraction <- extract(model_layers, vectors, ID = TRUE, xy = TRUE) %>% 
...
# vector_extraction <- vect(extraction_join)
...
lrn_eagle_tune <- lrn("classif.ranger", 
                   ...
                   mtry = to_tune(1, length(tsk_eagle$feature_types)),
                   importance = "impurity")
lrn_eagle_tune
```
...
df_design <- expand.grid(
  ...
  mtry = 1:length(tsk_eagle$feature_types))
...
rr <- resample(tsk_eagle, at, cv_outer, store_models = TRUE)
```
